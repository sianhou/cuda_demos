...
__global__ void sgemm(const float *a, const float *b, float *c, int M, int N, int K) {
    ...
    __shared__ __align__(16 * 1024) float shared_a[2 * STEP][STEP];
    __shared__ __align__(16 * 1024) float shared_b[STEP][2 * STEP];

    for (int kk = 0; kk < K; kk += 2 * STEP) {
        ...
        for (int ir = 0; ir < STRIDE; ++ir) {
            for (int ic = 0; ic < STRIDE; ++ic) {
                ...
                smA(row, col + STEP) = ptr_a[row + (col + STEP) * M];

                smB(row, col) = ptr_b[row + col * K];
                smB(row + STEP, col) = ptr_b[(row + STEP) + col * K];
            }
        }
        __syncthreads();
...
int main() {
    ...
    for (int i = 0; i < device_count; ++i) {
        ...
        {
            Result res;
            std::ofstream ofs(std::string(prop.name) + " sgemm_v5_blk8x8_stride2x2.txt");
        ...
        {
            Result res;
            std::ofstream ofs(std::string(prop.name) + " sgemm_v5_blk8x8_stride4x4.txt");
        ...
        {
            Result res;
            std::ofstream ofs(std::string(prop.name) + " sgemm_v5_blk16x16_stride2x2.txt");
...
//        {
//            Result res;
//            std::ofstream ofs(std::string(prop.name) + " sgemm_v5_blk16x16_stride4x4.txt");
//
//            for (int s = 64; s <= 4096; s *= 2) {
